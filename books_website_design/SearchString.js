function loadBook(bookPath, bookname) {
  let currentBook = "";
  let url = `books/${bookPath}`;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      let elmt = document.createElement("fileContent");
      elmt.scrollTop = 0;
    }
  };
}

function getDocStats(bookContent) {
  let docLength = document.getElementById("docLength");
  let wordCount = document.getElementById("wordCount");
  let charCount = document.getElementById("charCount");

  let text = bookContent.toLowerCase();
  ...
  let last5Words = wordList.slice(-6, wordList.length);

  ULTemplate(top5Words, document.getElementById("mostUsed"));
  ULTemplate(last5Words, document.getElementById("leastUsed"));
